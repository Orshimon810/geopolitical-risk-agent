...
def consistency_validator_node(state: DynamicAgentState) -> DynamicAgentState:
    ...
    prompt = (
        ...
        "Correct any genuine contradictions.\n\n"
        "INVESTOR TAKEAWAY:\n"
        ...
        f"{holdings_block}\n\n"
        "CONSISTENCY RULE:\n"
        "The verdict assigned to each holding must match the directional stance in the "
        "investor takeaway and market impacts.\n"
        "Examples of contradictions:\n"
        "- Takeaway says 'Reduce exposure to shipping equities' → a shipping stock marked Bullish is WRONG\n"
        "- Market impact says 'Oil exporters benefit from price spike' → oil ETF marked Bearish is WRONG\n"
        "- Takeaway says 'Rotate into defense stocks' → defense ETF marked Bearish is WRONG\n\n"
        "IMPORTANT — DO NOT flag these as contradictions:\n"
        "- A holding marked Neutral because it has no exposure (this is correct, not a contradiction)\n"
        "- Holdings in sectors not mentioned in the takeaway (absence of mention ≠ contradiction)\n"
        "- Disagreements with your own opinion (only flag explicit text↔verdict contradictions)\n\n"
        "Instructions:\n"
        "1. Check each holding's verdict against the investor takeaway and market impact bullets.\n"
        "2. Only flag genuine contradictions where the written text explicitly says one direction "
        "but the verdict says the opposite.\n"
        "3. For each contradiction: provide the corrected verdict, honest reasoning, and a brief "
        "description of the contradiction found.\n"
        "4. Set contradictions_found=true only if at least one genuine contradiction exists."
    )
