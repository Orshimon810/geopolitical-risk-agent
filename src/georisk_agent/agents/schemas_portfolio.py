...
class ExposureVector(BaseModel):
    """
    One directional exposure channel for a holding in a trade-policy-tariff event.
    Generated by the ticker analyst when event_type == "trade_policy_tariff".
    Used by _apply_trade_policy_balanced_verdict() in the reduce node.
    """
    channel: Literal[
        ...
        "indirect-demand",
        "none",
    ] = Field(description="Transmission channel for this exposure vector.")
    direction: Literal["positive", "negative", "neutral", "mixed"] = Field(
        description="Net directional impact through this channel."
    )
    materiality: Literal["none", "low", "medium", "high"] = Field(
        description="Relative size of this channel's impact on the holding."
    )
    confidence: Literal["high", "medium", "low", "insufficient_data"] = Field(
        description="Confidence in this vector given available evidence."
    )
    rationale: str = Field(
        description="1-2 sentences citing the specific mechanism for this vector."
    )
...
class MacroEventContext(BaseModel):
    ...
    event_type: Optional[str] = Field(
        default=None,
        description=(
            "Event type classification set programmatically by analysis_node — do not infer. "
            "'trade_policy_tariff': query describes tariffs, import/export duties, anti-dumping, "
            "or trade barriers. None for all other event types."
        ),
    )
...
class TickerHoldingAnalysis(BaseModel):
    ...
    # 6 ── Trade-policy exposure vectors (populated only when event_type=="trade_policy_tariff")
    exposure_vectors: list[ExposureVector] = Field(
        default_factory=list,
        description=(
            "Decomposed exposure channels for trade-policy-tariff events (RULE 10). "
            "Empty list for all other event types. Each vector names one transmission "
            "channel, its direction, materiality, confidence, and rationale."
        ),
    )
